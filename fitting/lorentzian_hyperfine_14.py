...
class Lorentzian_hyperfine_14(FitFunc):
    ...
    param_defn = ["fwhm", "pos", "amp_1_hyp", "amp_2_hyp", "amp_3_hyp"]
    parameter_unit = {
        "fwhm": "Frequency (MHz)",
        "pos": "Frequency (MHz)",
    ...
    @staticmethod
    @njit(fastmath=True)
    def base_function(x, fwhm, pos, amp_1_hyp, amp_2_hyp, amp_3_hyp):
        hwhmsqr = fwhm ** 2 / 4
        return (
            amp_1_hyp * hwhmsqr / ((x - pos - 2.14) ** 2 + hwhmsqr)
            + amp_2_hyp * hwhmsqr / ((x - pos) ** 2 + hwhmsqr)
            + amp_3_hyp * hwhmsqr / ((x - pos + 2.14) ** 2 + hwhmsqr)
        )
